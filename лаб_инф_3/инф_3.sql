...
SELECT d_name FROM dishes WHERE category_id = 2;
SELECT * FROM orders WHERE order_date >= '2025-11-16' AND order_date <= '2025-11-18';
SELECT SUM(total_price) as total_sum FROM orders WHERE order_date = '2025-11-16';
SELECT dishes.d_name, SUM(orders.quantity) as total_ordered FROM orders
JOIN dishes ON orders.dish_id = dishes.d_id
GROUP BY dishes.d_id, dishes.d_name
